...
df_tv = df[df["device"] == "tv"].reset_index(drop=True)

df_tv["time"] = range(1, len(df_tv) + 1)

plt.figure(figsize=(12, 6))
plt.plot(df_tv["time"], df_tv["cpu_usage"], label="CPU Usage (%)", color="red")
plt.plot(df_tv["time"], df_tv["mem_usage"], label="Memory Usage (MB)", color="blue")
plt.plot(df_tv["time"], df_tv["net_usage"], label="Network Usage (Mbps)", color="green")

plt.title("Smart TV Metrics Over Time")
